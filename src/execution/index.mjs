...
async function signBatch(ctx, wave, work) {
  ...
  for (const job of work) {
    ...
    try {
      ...
      await report(ctx, 'signer-result', { actionId: item.planned.id, signer: job.signer.address, signerLatencyMs: Date.now() - started });
    } catch (error) {
...
export async function applyPlan({ plan, spec, artifacts, client, signers, signerProvider, signerRoles, stateStore, journalStore, lockProvider, journalCipher, scope: scopeInput, principal, ttlMs, stateFile, journalFile, parallel = false, ...options }) {
  ...
  try {
    ...
    const ctx = { plan, spec, artifacts, client, lanes, deps, journal, lock, config, scope, remote, principal: lock.holder?.principal ?? principal, readState, writeState, stateFile: stateFile ?? null, parallel, spent: new Map(), sent: [], rebroadcasts: [], outcomes: new Map(), state: { file: stateFile ?? null, written: false } };
