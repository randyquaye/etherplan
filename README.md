...
Mark owner-only configuration calls with `"ownerOnly": true`. A call with method `transferOwnership`, or one marked `"transfersOwnership": true`, waits for all owner-only calls on the same target. Use `after` for ordering across targets or for other state dependencies. Add `executionAssumptions` as an array of reviewable text when a constructor safely stores a predicted address, for example `"constructor does not call contracts.registry"`. Etherplan warns about constructor address references and linked libraries without execution edges unless an assumption names that contract ID exactly. `validate`, `graph`, `plan`, and `schedule` show the warnings.
